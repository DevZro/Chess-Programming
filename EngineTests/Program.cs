...
namespace Chess // All methods, struts and classes related to the Chess Operations are stored in the Chess namespace. 
{
    /* 
    ...
    */
    public class ChessBoard
    {
        ...
        */
        ...
        private ulong occupied; // bitboard of all currently occupied square
        private ulong empty;  // bitboard of all empty squares but would probably go given it can be easily found as ~occupied
        private ulong whiteOccupied; // bitboard of all squares occupied by the white pieces
        private ulong blackOccupied; // bitboard of all squares occupied by the black pieces
        ...
        */
        ...
        const int regularFlag = 1;
        const int captureFlag = 2;
        const int castlingFlag = 3;
        const int doublePawnMoveFlag = 4;
        const int enpessantFlag = 5;
        
        const int knightPromotionFlag = 6;
        const int bishopPromotionFlag = 7;
        const int rookPromotionFlag = 8;
        const int queenPromotionFlag = 9;
        ...
        */
        ...
        /*
        The initialisation of an instance automatically sets the value of the bitboards, sets castlingRights as well as other state defining variables
        Eventually, this should work with the load Fen method but that remains to be implemented
        */
        public ChessBoard(string Fen=STARTING_POSITION)
        {
            LoadFen(Fen);
        ...
        */
        ...
        private void UpdateOccupiedAndEmpty()
        {
            whiteOccupied = 0;
            blackOccupied = 0;
        ...
        */
        ...
        private ulong GenerateKnightAttacks(bool isWhite)
        {
            ...
            while (true)
            {
                square = BitOperations.TrailingZeroCount(knights); // the square of the first knight is found
        ...
        */
        private (ulong, ulong) GetPinnedPieces(bool isWhite)
        {
            ...
            {
                square = BitOperations.TrailingZeroCount(pinningPieces);
                ...
                if ((BitOperations.TrailingZeroCount(inBetweenBitboard & playingPieces) + BitOperations.LeadingZeroCount(inBetweenBitboard & playingPieces)) == 63) 
                {
                    pinnedSquare = BitOperations.TrailingZeroCount(inBetweenBitboard & playingPieces); // the square of said pinned piece
            ...
            while (pinningPieces != 0)
            {
                ...
                if ((BitOperations.TrailingZeroCount(inBetweenBitboard & playingPieces) + BitOperations.LeadingZeroCount(inBetweenBitboard & playingPieces)) == 63)
                {
                    pinnedSquare = BitOperations.TrailingZeroCount(inBetweenBitboard & playingPieces);
                    pinnedPieces |= 1Ul << pinnedSquare;
        ...
        */
        public List<Move> GenerateMoves()
        {
            ...
            {
                kingMoves.Add(new Move(kingSquare, BitOperations.TrailingZeroCount(pieceDestinations), captureFlag));
                pieceDestinations -= 1UL << BitOperations.TrailingZeroCount(pieceDestinations);
            }
            ...
            while (pieceDestinations != 0)  
            {
                kingMoves.Add(new Move(kingSquare, BitOperations.TrailingZeroCount(pieceDestinations), regularFlag));
                pieceDestinations -= 1UL << BitOperations.TrailingZeroCount(pieceDestinations);
            }
            ...
            if (isWhite)
            {
                ...
                while (pieces != 0)
                {
                    currentpieceSquare = BitOperations.TrailingZeroCount(pieces);
                    ...
                    for (int i = 0; i < 2; i++) // add vertical moves
                    {
                        ...
                        moves.Add(new Move(currentpieceSquare, BitOperations.TrailingZeroCount(pieceDestinations), (i == 0) ? regularFlag : doublePawnMoveFlag));
                    ...
                    {
                        moves.Add(new Move(currentpieceSquare, BitOperations.TrailingZeroCount(pieceDestinations), captureFlag));
                    }
                ...
                while (pieces != 0)
                {
                    ...
                    if ((pieceDestinations & empty) != 0)
                    {
                        moves.Add(new Move(currentpieceSquare, BitOperations.TrailingZeroCount(pieceDestinations), regularFlag));
                    }
                ...
                while (pieces != 0)
                {
                    ...
                    if ((pieceDestinations & empty) != 0)
                    {
                        moves.Add(new Move(currentpieceSquare, BitOperations.TrailingZeroCount(pieceDestinations), knightPromotionFlag));
                        moves.Add(new Move(currentpieceSquare, BitOperations.TrailingZeroCount(pieceDestinations), bishopPromotionFlag));
                        moves.Add(new Move(currentpieceSquare, BitOperations.TrailingZeroCount(pieceDestinations), rookPromotionFlag));
                        moves.Add(new Move(currentpieceSquare, BitOperations.TrailingZeroCount(pieceDestinations), queenPromotionFlag));
                    }
                    ...
                    for (int i = 0; i < 2; i++) // considers enpessant for both possible pawns that could capture
                    {
                        ...
                        if (((currentpieceSquare % 8) != edgecheck) && (((1UL << currentpieceSquare) & bitboards[pieceIndex]) != 0))
                        {
                            if (((1UL << currentpieceSquare) & pinnedPieces) == 0) // if the pawn is NOT pinned
                            {
                                ...
                                currentHeavyPieceSquare = BitOperations.TrailingZeroCount(heavyPiecesonfifth) % 64; // mod 64 to stop the 64 as the output of 0
                                ...
                                if ((kingSquare >= 32) && (kingSquare < 40)) // if king is on fifth rank
                                {
                                    while (currentHeavyPieceSquare != 0)
                                    {
                                        ...
                                        currentHeavyPieceSquare = BitOperations.TrailingZeroCount(heavyPiecesonfifth) % 64;
                    ...
                    for (int i = 0; i < 2; i++) // considers enpessant for both possible pawns that could capture
                    {
                        ...
                        if (((currentpieceSquare % 8) != edgecheck) && (((1UL << currentpieceSquare) & bitboards[pieceIndex]) != 0))
                        {
                            if (((1UL << currentpieceSquare) & pinnedPieces) == 0) // if the pawn is NOT pinned
                            {
                                ...
                                currentHeavyPieceSquare = BitOperations.TrailingZeroCount(heavyPiecesonfourth) % 64; // mod 64 to stop the 64 as the output of 0
                                ...
                                if (kingSquare >= 24 && (kingSquare < 32)) // if king is on fourth rank
                                {
                                    while (currentHeavyPieceSquare != 0)
                                    {
                                        ...
                                        currentHeavyPieceSquare = BitOperations.TrailingZeroCount(heavyPiecesonfourth) % 64;
            ...
            while (pieces != 0)
            {
                ...
                while (pieceDestinations!= 0)
                {
                    finalSquare = BitOperations.TrailingZeroCount(pieceDestinations);
            ...
            if (numCheck == 1) // If there is a single check, then it is complex
            {
                ...
                {                        
                    ...
                    while(true)
                    {
                        targetSquare = BitOperations.TrailingZeroCount(targetPiece);
